...
class YoutubeService {
  YoutubeService({required http.Client client, required String apiKey})
    : _client = client,
      _apiKey = apiKey;
  final http.Client _client;
  final String _apiKey;

  Future<List<YoutubeVideo>> fetchVideosByChannel(
    String channelId, {
    int maxResults = 20,
  }) async {
    ...
    final uploadsResponse = await _client
        .get(Uri.parse(uploadsUrl))
        .timeout(const Duration(seconds: 15));
    print(
      'DEBUG Service: Channels response status: ${uploadsResponse.statusCode}',
    );
    ...
    if (items.isEmpty) {
      throw Exception(
        'Channel ID không hợp lệ hoặc không tồn tại (totalResults: 0)',
      );
    }
    ...
    final videosUrl =
        'https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=$uploadsPlaylistId&maxResults=$maxResults&key=$apiKey&order=date';
    print('DEBUG Service: Calling playlistItems API: $videosUrl');
    final videosResponse = await _client
        .get(Uri.parse(videosUrl))
        .timeout(const Duration(seconds: 15));
    print(
      'DEBUG Service: PlaylistItems response status: ${videosResponse.statusCode}',
    );
    ...
    print(
      'DEBUG Service: Parsed to ${parsedVideos.length} YoutubeVideo objects',
    );
    return parsedVideos;
  }

  Future<List<YoutubePlaylist>> fetchPlaylistsByChannel(
    String channelId, {
    int maxResults = 50,
  }) async {
    final apiKey = _apiKey;
    if (apiKey.isEmpty) throw Exception('Không tìm thấy API Key YouTube');
    ...
    return items
        .map<YoutubePlaylist>((item) => YoutubePlaylist.fromJson(item))
        .toList();
  }

  Future<List<YoutubeVideo>> fetchVideosByPlaylist(
    String playlistId, {
    int maxResults = 20,
  }) async {
    final apiKey = _apiKey;
    ...
    final url =
        'https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=$playlistId&maxResults=$maxResults&key=$apiKey&order=date';
    print(
      'DEBUG Service: Calling playlistItems API for playlist $playlistId: $url',
    );
    final response = await _client
        .get(Uri.parse(url))
        .timeout(const Duration(seconds: 15));
